...
function ProgramCard({ program }: { program: Program }) {
  ...
  const toggleArchive = async () => {
    try {
      ...
      toast.success(
        program.isArchived ? "Program restored" : "Program archived",
      );
    } catch (error) {
  ...
  const duplicateProgram = async () => {
    ...
    try {
      ...
      const daysSnap = await getDocs(
        collection(db, "programs", program.id, "days"),
      );
      for (const dayDoc of daysSnap.docs) {
        const dayData = dayDoc.data();
        const newDayRef = await addDoc(
          collection(db, "programs", newProgramRef.id, "days"),
          {
            title: dayData.title,
        ...
        const workoutsSnap = await getDocs(
          collection(db, "programs", program.id, "days", dayDoc.id, "workouts"),
        );
          ...
          <Link href={`/programs/${program.id}`}>
            <CardHeader className="p-4 md:p-6 flex flex-row md:flex-col items-center md:items-start gap-4 md:gap-0">
              ...
              <div className="min-w-0">
                ...
                <CardDescription className="line-clamp-1 md:line-clamp-2 text-[10px] md:text-xs font-bold text-slate-400 dark:text-slate-500">
                  {program.description}
                </CardDescription>
              </div>
            </CardHeader>
    ...
    <div className="container mx-auto py-10 px-4 min-h-screen">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl md:text-4xl font-black text-foreground uppercase tracking-tight flex items-center gap-3">
            <BookOpen className="size-6 md:size-8 text-primary dark:text-blue-500" />
            Programs
          </h1>
      ...
      <Tabs defaultValue="operational" className="w-full">
        <TabsList className={cn(
          ...
          >
            Operational ({activePrograms.length})
          </TabsTrigger>
          <TabsTrigger
            value="vault"
            className="rounded-full px-4 md:px-6 data-[state=active]:bg-primary data-[state=active]:text-white text-xs md:text-sm font-black uppercase tracking-tight dark:text-slate-400"
          >
            Archived Vault ({archivedPrograms.length})
          </TabsTrigger>
        </TabsList>
