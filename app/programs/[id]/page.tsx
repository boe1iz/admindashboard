...
export function WorkoutCard({
  ...
  const deleteWorkout = async () => {
    try {
      await deleteDoc(
        doc(db, "programs", programId, "days", dayId, "workouts", workout.id),
      );
  ...
  const assignedEquipment =
    workout.equipment_ids
      ?.map((id) => equipment.find((e) => e.value === id)?.label)
      .filter(Boolean) || [];
              ...
              {assignedEquipment.length > 0 && (
                <div className="flex gap-1">
                  {assignedEquipment.slice(0, 2).map((name, i) => (
                    <span
                      key={i}
                      className="text-[7px] md:text-[8px] font-black uppercase tracking-widest bg-slate-200/50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-1 md:px-1.5 py-0.5 rounded-sm whitespace-nowrap"
                    >
                      {name}
                    </span>
                  ))}
                  {assignedEquipment.length > 2 && (
...
function DaySection({
  ...
  useEffect(() => {
    const q = query(
      collection(db, "programs", programId, "days", day.id, "workouts"),
      orderBy("order_index"),
    );
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Card className="group overflow-hidden border-slate-200 dark:border-slate-800 shadow-md rounded-3xl md:rounded-[40px] bg-card">
        <CardHeader className="flex flex-row items-center justify-between p-4 md:p-6 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            >
              <ChevronDown
                className={cn(
                  "size-6 md:size-5 text-slate-500 dark:text-slate-400 transition-transform duration-200",
                  !isExpanded && "-rotate-90",
                )}
              />
            </Button>
            <div>
              <CardTitle className="text-lg md:text-xl font-black text-foreground uppercase tracking-tight">
                {day.title}
              </CardTitle>
              {!isExpanded && (
...
export default function ProgramDetailPage({
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "programs", id, "days"),
      orderBy("day_number"),
    );
  ...
  if (loading)
    return (
      <div className="p-8 text-slate-500 font-black text-xs uppercase tracking-[0.2em] animate-pulse">
        Synchronizing Data...
      </div>
    );
  if (!program)
    return (
      <div className="p-8 text-foreground font-black uppercase">
        Program not found.
      </div>
    );
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-6">
          <div>
            <h1 className="text-2xl md:text-5xl font-black text-foreground uppercase tracking-tight flex items-center gap-4">
              <BookOpen className="size-8 md:size-10 text-primary dark:text-blue-500" />
              {program.name}
            </h1>
