...
export function ClientCard({
  ...
  const clientAssignments = assignments.filter(
    (a) => a.clientId === client.id || a.client_id === client.id,
  );
          ...
          <CardHeader className="p-4 md:p-6 flex flex-row md:flex-col items-center md:items-start gap-4 md:gap-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <CardTitle className="font-black text-foreground uppercase tracking-tight text-sm md:text-base truncate">
                  {client.name}
                </CardTitle>
                {isAdmin && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-400/20 text-amber-600 dark:bg-amber-400/15 dark:text-amber-400 text-[9px] font-black uppercase tracking-widest rounded-full border border-amber-400/40 shrink-0">
                    <ShieldCheck className="size-2.5" />
                    Admin
                  </span>
                )}
              </div>
              <CardDescription className="text-[10px] md:text-xs font-bold text-slate-400 dark:text-slate-500 truncate">
                {client.email}
              </CardDescription>
            </div>
          </CardHeader>
            ...
            <div className="flex flex-wrap gap-2">
              {clientAssignments.length > 0 ? (
                clientAssignments.map((assignment) => {
                  ...
                  const programName =
                    programs.find((p) => p.id === progId)?.name ||
                    progId ||
                    "Unknown Program";
                  return (
                    <span
                      key={assignment.id}
                      className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-full"
                    >
                      {programName}
                    </span>
                  );
...
export default function ClientsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAdmins = onSnapshot(
      ...
      (snapshot) => {
        setAdminUids(new Set(snapshot.docs.map((d) => d.id)));
      },
    );
        ...
        <TabsContent value="operational" className="space-y-4 outline-none">
          {activeClients.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {activeClients.map((client) => (
                <ClientCard
                  ...
                  programs={programs}
                  assignments={assignments}
                  isAdmin={adminUids.has(client.id)}
                />
              ))}
            </div>
          ) : (
            <Card className="p-8 md:p-12 border-slate-200 dark:border-slate-800 bg-card flex flex-col items-center justify-center text-center rounded-[24px] md:rounded-[40px] shadow-md">
              ...
              <CardTitle className="text-foreground font-black uppercase tracking-tight mb-2 text-lg md:text-xl">
                No Active Clients
              </CardTitle>
              <CardDescription className="font-medium text-slate-500 dark:text-slate-400 text-sm">
                Registered clients from the mobile app will appear here.
              </CardDescription>
            </Card>
          )}
        </TabsContent>
